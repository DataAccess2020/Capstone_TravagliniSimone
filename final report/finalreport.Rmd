...
After having taken all the hashtags of all the coalitions, the next step is that of the actual analysis. in order to analyze I decided to create wordclouds from the tables of the most used words as hashtags. these tables show, in ascending order, the hashtags most used by the coalitions during the electoral campaign period.
```{r, eval=FALSE}
...
wordcloud(words = df2$word, freq = df2$freq, min.freq = 6,
...
Another fundamental point of the electoral campaign for the left was the right for homosexuals and the lgbt community. as seen in the wordcloud among the hashtags there are "egalitarian marriage" "lgbt" and "rights".
...
The graphs show that the words most used by right-wing coalition politicians are those referring to voting for a reference party (lega, fdi and forza italia), but there are also hashtags referring to the main points of their electoral campaign (presidentialism, flat tax). one of the main ones is the hashtag #credo, launched precisely on the occasion of the elections by the leader of the Lega Matteo Salvini.
...
Although the list was large enough, I was only able to export 2760 tweets.
However we can say that, from the analysis of these hashtags, the one most used by far was "on the right side". 
secondly, the others were those referring to basic income and the minimum wage, but also young people and expensive bills. also in this case we find references to political opponents, first of all Meloni and Calenda.
...
wordcloud(words = df4$word, freq = df4$freq, min.freq = 2,
          max.words=Inf, random.order=FALSE, rot.per=0.35,
          colors=brewer.pal(10, "Dark2"),
          scale=c(2,0.60))
```

The slogan of the third pole coalition for the 2022 elections was "italia seriously", which was one of the most used hashtags in the period. in addition to this, the candidates expressed their views on infrastructure, health and justice reform. also in this case we find references to political opponents, but in this circumstance, the candidates of the third pole referred above all to Berlusconi and Di Maio.
